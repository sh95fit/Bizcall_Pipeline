...
class GeminiAIProvider(AIProvider):
    ...
    def _build_category_guide(self, categories: list) -> str:
        """
        현재: categories 테이블 name + 하드코딩 description 사용
        추후: categories 테이블 description 컬럼 조회로 교체 예정
        """
...
- transcript는 오디오에서 들리는 내용을 최대한 정확하게 변환하세요"""
    ...
    def analyze(self, categories: list, transcript: str | None = None, file_path: str | None = None) -> tuple[str | None, dict]:
        if transcript:
            ...
            return transcript, json.loads(response.text.strip())
        elif file_path:
            ...
            extracted_transcript = result.pop("transcript", None)
            return extracted_transcript, result
        else:
